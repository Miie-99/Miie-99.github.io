...
const DATA = {
    ...
    triggers: [
        // ========== CP热度触发器（冷圈 → 烫门） ==========
        // 极冷圈：cpHeat < 500（新量级：北极圈）
        {
            condition: (s) => s.cpHeat < 500,
            event: {
                title: "极地求生",
                text: "你的CP实在是太冷了，冷到Tag里上一条微博还是你半个月前发的。你在超话里大喊一声，只有回音。",
                effect: { passion: -10, san: -5, myHeat: -200 }
            }
        },
        // 冷圈：cpHeat < 5000（新量级）
        {
            condition: (s) => s.cpHeat < 5000 && s.cpHeat >= 500 && State.turn % 4 === 0,
            event: {
                title: "圈地自萌",
        ...
        // 冷圈惊喜：cpHeat < 10000 稀有正向
        {
            condition: (s) => s.cpHeat < 10000 && Math.random() < 0.08,
            event: {
                title: "冷圈奇迹",
        ...
        // 中等热度：cpHeat 100k-500k 官方偶尔互动
        {
            condition: (s) => s.cpHeat >= 100000 && s.cpHeat <= 500000 && Math.random() < 0.15,
            event: {
                title: "官方小饼干",
        ...
        // 高热度：cpHeat 500k-5M 官方频繁发糖
        {
            condition: (s) => s.cpHeat > 500000 && s.cpHeat <= 5000000 && Math.random() < 0.2,
            event: {
                title: "官方营业",
        ...
        // 大热门：cpHeat > 5M 同人本热销
        {
            condition: (s) => s.cpHeat > 5000000 && Math.random() < 0.15,
            event: {
                title: "流量红利",
                text: '因为CP正当红，你随手画的一个鱼摸居然有几千热度！大量的点赞提示音让你产生了一种这就是"大手"的错觉。',
                effect: { myHeat: 50000, passion: 10, tech: 2 }
            }
        },
        {
            condition: (s) => s.cpHeat > 5000000 && s.tech > 30 && Math.random() < 0.12,
            event: {
                title: "本子大卖",
                text: "趁着CP热度，你开了预售！订单量超出预期，印厂加急，你既开心又忙得脚不沾地。",
                effect: { money: 800, myHeat: 30000, stamina: -15 }
            }
        },
        // 顶流：cpHeat > 20M 是非多
        {
            condition: (s) => s.cpHeat > 20000000 && Math.random() < 0.2,
            event: {
                title: "烫门是非多",
        ...
        {
            condition: (s) => s.cpHeat > 20000000 && Math.random() < 0.1,
            event: {
                title: "拆家入侵",
                text: "你的CP热度太高，引来了一群专职拆CP的黑子。他们到处刷'官方没说是爱情'、'你们在做梦'，搅得圈内乌烟瘴气。",
                effect: { san: -25, passion: -10, cpHeat: -500000 }
            }
        },
        ...
        // 小透明：myHeat < 5000
        {
            condition: (s) => s.myHeat < 5000 && Math.random() < 0.15,
            event: {
                title: "小透明日常",
        ...
        {
            condition: (s) => s.myHeat < 2000 && s.love > 60 && Math.random() < 0.1,
            event: {
                title: "被遗忘的神作",
        ...
        // 新人期：myHeat 5k-50k
        {
            condition: (s) => s.myHeat >= 5000 && s.myHeat <= 50000 && Math.random() < 0.12,
            event: {
                title: "初次被转发",
                text: "一个粉丝数比你多的太太转发了你的图，评价是'很有潜力'。虽然是八百年前的客套话，但你还是开心了一整天。",
                effect: { passion: 15, myHeat: 5000, san: 5 }
            }
        },
        // 中V期：myHeat 50k-500k
        {
            condition: (s) => s.myHeat >= 50000 && s.myHeat <= 500000 && Math.random() < 0.1,
            event: {
                title: "约稿邀请",
        ...
        {
            condition: (s) => s.myHeat >= 100000 && s.myHeat <= 1000000 && Math.random() < 0.08,
            event: {
                title: "首页安利",
                text: "你的作品被列入了'本周神作推荐'！评论区涌入一堆新面孔，你一边回复评论一边傻笑。",
                effect: { myHeat: 50000, social: 15, passion: 15 }
            }
        },
        // 大V期：myHeat > 500k
        {
            condition: (s) => s.myHeat > 500000 && s.myHeat <= 2000000 && Math.random() < 0.1,
            event: {
                title: "同行嫉妒",
        ...
        {
            condition: (s) => s.myHeat > 1000000 && Math.random() < 0.08,
            event: {
                title: "商业邀约",
        ...
        // 顶流：myHeat > 5M
        {
            condition: (s) => s.myHeat > 5000000 && Math.random() < 0.12,
            event: {
                title: "私生困扰",
        ...
        {
            condition: (s) => s.myHeat > 5000000 && Math.random() < 0.1,
            event: {
                title: "被挂了",
                text: "因为你说了一句中立的话，被截图挂到了广场上。支持你的和反对你的吵成一团，你这才意识到人红是非多。",
                effect: { san: -25, myHeat: 100000 }
            }
        },
        {
            condition: (s) => s.myHeat > 10000000 && !s.goddess,
        ...
        // CP热 + 个人热 = 爆发
        {
            condition: (s) => s.cpHeat > 5000000 && s.myHeat > 500000 && Math.random() < 0.08,
            event: {
                title: "天时地利",
                text: "CP正热+你也正红，你发的每条内容都能引爆首页！接下来的创作会事半功倍。",
                effect: { passion: 30, myHeat: 100000, love: 10 }
            }
        },
        // CP冷 + 个人热 = 拉动效应
        {
            condition: (s) => s.cpHeat < 50000 && s.myHeat > 300000 && Math.random() < 0.1,
            event: {
                title: "一人成军",
                text: "虽然CP没什么热度，但因为你的影响力，越来越多人开始关注这对CP了。你就是这个冷圈的太阳！",
                effect: { cpHeat: 50000, passion: 20, love: 15 }
            }
        },
        ...
        // ========== 原有属性触发器 ==========
        {
            ...
            event: { title: "发疯文学", text: "你的精神状态已岌岌可危，在微博连发50条乱码，吓跑了粉丝。", effect: { myHeat: -50000, social: -10 } }
        },
        {
    ...
    news: [
        // 圈内动态
        { text: '【快讯】某知名太太宣布封笔退圈，粉丝泪目', type: 'alert' },
        { text: '【热搜】{cp}相关tag冲上热搜第一！', type: 'hot' },
        { text: '【速报】官方突然发糖，全网沸腾中', type: 'happy' },
        { text: '【战报】CP粉与逆CP粉在广场大战，双方各有伤亡', type: 'alert' },
        { text: '【公告】新番PV发布，{cp}名场面截图刷屏', type: 'hot' },
        { text: '【争议】某画师因AI争议删除全部作品', type: 'alert' },
        { text: '【喜讯】同人展圆满落幕，{cp}场子人气爆棚', type: 'happy' },
        { text: '【预警】某太太疑似被盗号，粉丝紧急备份产出', type: 'alert' },
        { text: '【热评】最新一话评论两极分化，你站哪边？', type: 'hot' },
        { text: '【快讯】官方联动活动开启，限定谷子预售秒空', type: 'hot' },

        // 真实风格新闻
        { text: '【社会】又有家长投诉孩子沉迷追星，专家呼吁理性', type: 'normal' },
        { text: '【文娱】某明星工作室发声明澄清恋情传闻', type: 'normal' },
        { text: '【科技】B站服务器疑似崩溃，用户集体吐槽', type: 'alert' },
        { text: '【财经】二次元经济持续升温，周边市场规模破千亿', type: 'normal' },
        { text: '【热搜】"磕到了"登上热搜，网友：懂的都懂', type: 'happy' },
        { text: '【公告】lofter系统维护中，同人文暂时无法访问', type: 'alert' },
        { text: '【速报】AO3无法访问，圈内哀鸿遍野', type: 'alert' },
        { text: '【喜讯】某CP同人小说被改编成广播剧', type: 'happy' },
        { text: '【热评】"年度最甜CP"投票开启，{cp}暂列第一', type: 'hot' },
        { text: '【快讯】动漫节开幕在即，Coser准备就绪', type: 'happy' },

        // 圈内梗
        { text: '【调查】你一周在同人上花多少时间？评论区炸了', type: 'normal' },
        { text: '【名场面】太太直播画{cp}，观众刷礼物刷到手软', type: 'happy' },
        { text: '【争议】"OOC"定义引发激烈讨论，双方各执一词', type: 'alert' },
        { text: '【快讯】某太太连更30天，粉丝：太太永远的神', type: 'happy' },
        { text: '【热搜】#同人女的日常# 话题阅读量破亿', type: 'hot' },
        { text: '【公告】某同人志印刷延期，主催发长文道歉', type: 'alert' },
        { text: '【速报】圈内大手互关，粉丝：我嗑到了', type: 'happy' },
        { text: '【热评】"冷圈自萌"话题引发共鸣，评论区泪目', type: 'normal' },
        { text: '【快讯】官方暗示新剧情，CP粉连夜考据', type: 'hot' },
        { text: '【喜讯】{cp}同人曲播放量突破100万', type: 'happy' }
    ],

    // 获取随机新闻
    getRandomNews(count = 5) {
        const shuffled = [...this.news].sort(() => Math.random() - 0.5);
        return shuffled.slice(0, count).map(item => ({
            ...item,
            text: item.text.replace(/{cp}/g, State.cp).replace(/{rival}/g, State.rival)
        }));
    }
};
